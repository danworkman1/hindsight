...
async function runAgent({
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if (response.stop_reason === "end_turn") {
      const textBlock2 = response.content.find((b) => b.type === "text");
    ...
    if (response.stop_reason === "tool_use") {
      ...
      for (const block of response.content) {
        ...
        const handler = toolHandlers2[block.name];
        let result;
        if (!handler) {
...
var MAX_DIFF_BYTES = 2e4;
var tools = [
  {
    name: "git_diff",
    description: "Get the git diff of recent changes in the working tree. Use this to see what code was added or modified in the current session.",
    input_schema: {
      type: "object",
      properties: {
        staged: {
          type: "boolean",
          description: "If true, show staged changes only. If false (default), show all working-tree changes vs HEAD."
        }
      }
    }
  },
  {
    name: "read_file",
    description: "Read the full contents of a file. Use this when the diff doesn't give you enough surrounding context to judge a change.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "List files that have been modified, added, or deleted in the working tree. Useful as a first step to see the scope of changes.",
    input_schema: { type: "object", properties: {} }
  }
];
function createToolHandlers(base = null) {
  ...
  return {
    ...
    read_file: ({ path: path2 }) => {
      try {
        ...
        return `Error reading ${path2}: ${err.message}`;
      }
    },
    list_changed_files: () => {
      try {
        const out = range ? execSync(`git diff ${range} --name-status`, { encoding: "utf-8" }) : execSync("git status --porcelain", { encoding: "utf-8" });
        return out || "(no changes)";
      } catch (err) {
...
import { writeFileSync, readFileSync as readFileSync2, existsSync, statSync, mkdirSync } from "fs";
...
function computeCommitRangeHash(base = "HEAD~1") {
  try {
  ...
  let commitSha = "";
  try {
    commitSha = execSync3("git rev-parse HEAD", {
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    }).trim();
  } catch {
  ...
  if (base === "HEAD~1") {
    try {
      execSync3("git rev-parse HEAD~1", { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
    } catch {
      return { status: "no_parent" };
    }
  }
  ...
  let diff = "";
  try {
    diff = execFileSync("git", ["diff", range, "--", ".", ...DOC_EXCLUDE_PATHSPECS], {
      encoding: "utf-8",
      stdio: ["pipe", "pipe", "pipe"]
    });
  } catch {
...
function getCachedReview(hash) {
  if (!hash) return null;
  const cache = loadCache();
  return cache.entries[hash] || null;
}
function setCachedReview(hash, { changed, summary, verdict, prose, files, suggestions, branch, commitSha }) {
  if (!hash) return;
  const cache = loadCache();
  cache.entries[hash] = {
    changed,
...
function getBranchReviewCount(branch) {
  if (!branch) return 0;
  const cache = loadCache();
  return (cache.branchIndex[branch] || []).length;
}
function getLastBranchReview(branch) {
...
// lib/prior-review.js
function formatPriorReviewForPrompt(prior) {
  ...
  lines.push("");
  lines.push(
    "IMPORTANT: The prior review may have been wrong, or the code may have changed since. Reassess independently. If you agree with the prior verdict and the diff is minor, restate the prior conclusion. If you disagree, explain what changed your mind. Do not relitigate points the prior review already addressed unless the new diff makes them relevant again."
  );
  return lines.join("\n");
}
...
function renderReview({ ts, project, tag, summary, verdict, prose, files, suggestions }) {
  if (verdict === "clean") {
    return `
...
function logError(kind, message, detail) {
  ensureLogDir();
  const ts = (/* @__PURE__ */ new Date()).toISOString();
  const lines = [`[${ts}] [${projectTag()}] [error:${kind}] ${message}`];
  if (detail) {
    lines.push(DIVIDER, String(detail), DIVIDER);
  }
  lines.push("");
  appendFileSync(getLogPath(), lines.join("\n"));
}
function logReview({ summary, verdict, prose, files, suggestions, fromCache = false }) {
  ensureLogDir();
...
// lib/lock.js
import { existsSync as existsSync3, readFileSync as readFileSync3, writeFileSync as writeFileSync2, unlinkSync } from "fs";
import { tmpdir } from "os";
import { join as join2 } from "path";
import { createHash as createHash2 } from "crypto";
var STALE_AFTER_MS = 5 * 60 * 1e3;
function getLockPath() {
  const repoKey = createHash2("sha256").update(process.cwd()).digest("hex").slice(0, 16);
  return join2(tmpdir(), `hindsight-${repoKey}.lock`);
}
function acquireLock() {
  const path2 = getLockPath();
  if (existsSync3(path2)) {
    const age = Date.now() - parseInt(readFileSync3(path2, "utf-8") || "0", 10);
    if (age < STALE_AFTER_MS) return false;
    logSkip("lock", `reclaiming stale lock (${Math.round(age / 1e3)}s old)`);
  }
  writeFileSync2(path2, String(Date.now()), "utf-8");
  return true;
}
...
function readCommitMetadata(sha) {
  try {
    const branch = execSync4("git rev-parse --abbrev-ref HEAD", { encoding: "utf-8" }).trim();
    const message = execSync4("git log -1 --pretty=%B", { encoding: "utf-8" }).trim();
    return { branch, message, sha };
  } catch {
    return { branch: "", message: "", sha: "" };
  }
}
async function triage(toolHandlers2, model = MODELS.HAIKU) {
...
- If unsure, lean toward false \u2014 Phase 2 is expensive`;
  const result = await runAgent({
    system,
    userPrompt: "Inspect the working tree. Was source code added or refactored in this session?",
    tools,
    toolHandlers: toolHandlers2,
    maxIterations: 5,
    model
